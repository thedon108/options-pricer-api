...
def bs_iv(market_price, S, K, T, r, option_type='call', tol=1e-6, max_iter=100):
    ...
    if T <= 0 or market_price <= 0:
        return None
    # European lower bound (not American intrinsic — European put can price below K-S)
    disc = math.exp(-r * T)
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == '/api/quote':
                ...
                # Rolling annualised HV for windows 20, 30, 60
                ...
                # Build date-indexed series for the chart (skip NaN rows)
                valid = hv30.dropna()
                dates  = [d.strftime('%Y-%m-%d') for d in valid.index]
                hv30_s = [round(float(v), 4) for v in valid.values]
                hv20_v = hv20.dropna()
                hv60_v = hv60.dropna()
                # Current snapshots (last valid value)
                ...
                # ATM IV from nearest-expiry chain
                atm_iv = None
                try:
